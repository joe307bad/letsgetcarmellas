...
{
    return 'id_' . (time() . mt_rand(9000, 9999) . '');
}
    ...
    function get_selected_pf_images($gt3_theme_pagebuilder)
    {
        if (is_array($gt3_theme_pagebuilder['post-formats']['images'])) {
            ...
            if (is_array($gt3_theme_pagebuilder['post-formats']['images'])) {
                foreach ($gt3_theme_pagebuilder['post-formats']['images'] as $imgid => $img) {
                    $compile .= '
                        <img src="' . aq_resize(wp_get_attachment_url($img['attach_id']), "1170", "770", true, true, true) . '" data-thumb="' . aq_resize(wp_get_attachment_url($img['attach_id']), "1170", "770", true, true, true) . '" alt="" />
                    ';
                }
            }
...
if (!function_exists('get_module_html')) {
    function get_module_html()
    {
        ...
        if ($module_name == "bg_start" || $module_name == "bg_end") {
            $module_size = "block_1_1 block_fullsize";
            $size_caption = "1/1";
        }
                            ...
                            <div class='this-option img-in-slider'>
                                <div class='padding-cont'>
                                    <div class='fl w9'>
                                        ...
                                        <input name='pagebuilder[sliders][fullscreen][slides][{$key}][title][value]' type='text' value='".esc_attr($slide['title']['value'])."' class='textoption type1'>
                                    </div>
                        ...
                        <div class='edit_popup'>
                            ...
                            <div class='this-option img-in-slider'>
                                <div class='padding-cont'>
                                    <div class='fl w9'>
                                        <h4>" . __('Title', 'gt3_builder') . "</h4>
                                        <input name='pagebuilder[sliders][fullwidth][slides][{$key}][title][value]' type='text' value='".esc_attr($slide['title']['value'])."' class='textoption type1'>
                                    </div>
                                    <div class='right_block fl w1'>
                                        <h4>" . __('color', 'gt3_builder') . "</h4>
...
function get_plugin_pagination($range = 10, $type = "")
{
    ...
    if ($type == "show_in_shortcodes") {
        ...
        $gt3_wp_query = $wp_query_in_shortcodes;
    } else {
        global $paged, $gt3_wp_query;
    }
    ...
    $max_page = $gt3_wp_query->max_num_pages;

...
function get_module_settings_part_heading($gt3_theme_pagebuilder, $moduleid, $pb_module_size_now)
{
    ...
    if (!isset($gt3_theme_pagebuilder['modules'][$moduleid]['subtitle_text'])) {
        $gt3_theme_pagebuilder['modules'][$moduleid]['subtitle_text'] = '';
    }
    ...
    if (!isset($gt3_theme_pagebuilder['modules'][$moduleid]['heading_style'])) {
        $gt3_theme_pagebuilder['modules'][$moduleid]['heading_style'] = 'yes';
    }

    return "
    <div class='padding-cont'>
        <div class='heading gt3_heading_settings'>
            <h4>" . __('Heading', 'gt3_builder') . "</h4>
            <input type='text' class='textoption type1 gt3_popup_heading' value='".esc_attr($gt3_theme_pagebuilder['modules'][$moduleid]['heading_text'])."' name='pagebuilder[modules][$moduleid][heading_text]'>
        </div>
        ...
        <div class='clear'></div>  	
		<div class='gt3_subtitle_settings mb20'>
            <h4>" . __('Sub Title', 'gt3_builder') . "</h4>
            <input type='text' class='textoption type1 gt3_popup_subtitle' value='".esc_attr($gt3_theme_pagebuilder['modules'][$moduleid]['subtitle_text'])."' name='pagebuilder[modules][$moduleid][subtitle_text]'>		
		</div>
        <div>
          <h4>" . __('Show Title Divider', 'gt3_builder') . "</h4>
          " . toggle_radio_yes_no("pagebuilder[modules][$moduleid][heading_style]", (isset($gt3_theme_pagebuilder['modules'][$moduleid]['heading_style']) ? $gt3_theme_pagebuilder['modules'][$moduleid]['heading_style'] : 'yes'), "") . "
...
function get_module_settings_part_media_attach_id_upload($gt3_theme_pagebuilder, $moduleid, $pb_module_size_now, $temp, $headingtext, $settingname)
{
    return "
    <div class='enter_option_row upload_file_cont'>
        <h5>$headingtext</h5> 
        <div class= 'img-item style_small img_upload_preview'>
            <div class= 'img-preview'>
                <div class='attach_id_added'>
                " . (isset($gt3_theme_pagebuilder['modules'][$moduleid][$settingname]) ? "<img src='" . (aq_resize($gt3_theme_pagebuilder['modules'][$moduleid][$settingname], "60", "60", true, true, true)) . "' alt=''>" : '') . "         
        ...
        <div class='for_post_fomrats img-item style_small gt3_add_media_attach cboxElement' name='pagebuilder[modules][$moduleid][$settingname]'>
            <div class='img-preview'>
               <img alt='' src='" . PBIMGURL . "/add_image.png'>
            </div>
            <input class='gt3_image_selected_id' name='pagebuilder[modules][$moduleid][$settingname]' type='hidden' value='" . (isset($gt3_theme_pagebuilder['modules'][$moduleid][$settingname]) ? $gt3_theme_pagebuilder['modules'][$moduleid][$settingname] : '') . "' />
        </div><!-- .img-item -->
        <div class='clear'></div>                            
    </div>    
...
function gt3_get_parameters_array($cpt)
{
    $args = array('taxonomy' => 'Category');
    ...
    if (is_array($terms) && count($terms) > 0) {
        foreach ($terms as $term) {
            ...
            $temp_key = $term->slug;
            $temp_value = $term->name;
            array_push($key, $temp_key);
...
function get_module_settings_part_icons($gt3_theme_pagebuilder, $moduleid, $headingtext, $settingname)
{
    ...
    foreach ($GLOBALS["pbconfig"]['all_available_font_icons'] as $icon) {
        $compile .= "<li class='stand_social'><i data-icon-code='" . $icon . "' class='stand_icon " . $icon . "'></i></li>";
    }
    ...
    if (isset($gt3_theme_pagebuilder['modules'][$moduleid][$settingname]) && is_array($gt3_theme_pagebuilder['modules'][$moduleid][$settingname])) {
        foreach ($gt3_theme_pagebuilder['modules'][$moduleid][$settingname] as $key => $value) {
            ...
            <div class='stand_iconsweet ui-state-default'>
                ...
                <input class='cpicker textoption type1' type='text' name='pagebuilder[modules][" . $moduleid . "][" . $settingname . "][" . $key . "][fcolor]' value='" . $value['fcolor'] . "' placeholder='" . __('Icon color', 'gt3_builder') . "'>
